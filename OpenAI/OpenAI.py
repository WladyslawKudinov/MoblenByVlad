...
def create_responce(variables, stage):
    ...
    if stage == 1:
        stage_prompt = "Ответ школьника такой: " + variables["project_info"] + (
                                                                                     ...
                                                                                     "Но не пиши '''json в начало и в конец, просто вставь текст в json формате. Не здоровайся, у вас идет диалог.")
    elif stage == 2:
        stage_prompt = "Вопросы были такими: " + variables["questions"] + "Ответ школьника такой: " + variables["detalization_answers"] + ("Теперь тебе нужно понять базовые ошибки школьника в его бизнес-решениях, бизнесе в целом, нехватка его данных. Тебе нужно понять, учитывая сам проект, его статус и так далее, а также ответы"
                                                                                                                                           ...
                                                                                                                                           "стартапа. Не пиши душно, ты все еще Зари. Пиши кратко и содержательно. Без воды. В этом сообщении не вставляй задания для школьника. Сделаешь это в следующих. Закончи свое сообщение фразой, которая будет говорить о том, что ты хочешь дать школьнику задания для оттачивания его навыков. Не здоровайся, у вас идет диалог."
                                                                                                                                           )
        ...
        stage_prompt = ("Итак, у тебя есть все ответы на все твои 4 вопроса. Вот они: Вопрос 1: " + variables["task1"] + " Ответ 1: " + variables["answer1"] + " Вопрос 2: " + variables["task2"] + " Ответ 2: " + variables["answer1"] + "  Вопрос 3: " + variables["task3"] + "  Ответ 3: "+ variables["answer1"] +"  Вопрос 4:" + variables["task4"] + " Ответ 4: " + variables["answer1"] +
                        ...
                        "Пиши не сильно структурировано. Пиши как дружелюбная Зари в диалоге в соцсетях, как бы ты помогаешь другу. Но это ваше последнее сообщение, напиши что на этом у тебя все и попрощайся. Не здоровайся, у вас идет диалог.")
    ...
    if stage == 1:
        response = client.chat.completions.create(
            model="gpt-4o-mini",
            messages=[
                {"role": "system", "content": f"Тебя зовут Заря, ты опытная девушка-бизнесмен. Сейчас ты будешь общаться со школьником, чтобы помочь ему со стартап-проектом. Помогай ему конкретными примерами, используй смайлики, и будь дружелюбной и мотивирующей!"},
